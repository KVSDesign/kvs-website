...
export const ProjectItem = ({ project, selected, onSelect }: ProjectItemProps) => {
    ...
    return (
        <div ref={divRef} className="flex flex-col gap-6" data-project-item={project.slug} id={project.slug}>
            ...
            <Link href={`#${project.slug}`} className={`${selected !== null ? 'pointer-events-none' : ''}`} onClick={handleClick}>
                <div ref={imageContainerRef} className="w-full h-[641px] rounded-[12px] bg-no-repeat bg-center bg-cover flex flex-col items-center justify-center gap-2 relative group" style={{ backgroundImage: `url(${project.thumbnailUrl})` }}>
                    ...
                    <div
                        ...
                        className={cn('absolute h-[250px] overflow-hidden scale-0 opacity-0 flex items-center flex-col justify-center z-10 transition-all duration-400 ease-in-out group-hover:scale-100 group-hover:opacity-100', project.lotties[0].larger ? 'h-[550px]' : '', selected === true ? '!scale-100 !opacity-100' : '')}
                    >
                        {
                            selected === true && (
            ...
            </Link>
            {
                selected === true && (
                    project.lotties.map((lottie, i) => {
                        return (
                            <div key={`project-${project.slug}-lottie-${i}`} className="h-[641px]">
                                <div className="w-full h-full bg-[#161314] rounded-[12px] flex flex-col items-center justify-center gap-2">
                                    <div className={cn('h-[250px] overflow-hidden flex items-center flex-col justify-center', lottie.larger ? 'h-[550px]' : '')}>
